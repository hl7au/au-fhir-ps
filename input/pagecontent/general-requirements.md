...
- functions as a referral patient summary attachment viewer and has received an AU Patient Summary to support a cardiology referral for palpitations and dizziness on exertion for a 50 year old female patient with history of hypertension on anti-hypertensive drugs (e.g. Enalapril 10 mg daily), and Grade II oligodendroglioma 6 years ago  needs to consider the value of `MedicationStatement.medication[x]`. This content is considered relevant when presented as it may be clinically important to the referred clinician, such as:
...
- functions as a patient summary attachment viewer and has received an AU Patient Summary to support dental implant treatment where the patient summary comtains both a refuted penicillin allergy and a confirmed, active nut allergy, needs to consider the values of `AllergyIntolerance.code`, `AllergyIntolerance.verificationStatus`, and `AllergyIntolerance.clinicalStatus`. In this context
  ...
  - for a confirmed, active nut allergy (`AllergyIntolerance.code` with `AllergyIntolerance.clinicalStatus` = "active" and `AllergyIntolerance.verificationStatus` = "confirmed"), the consumer cannot ignore these values and is needs to consider them as part of the AU PS content, even if the system later chooses not to prioritise this information for the dental implant use case.
